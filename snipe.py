...
def custom_info(message):
    logging.info(f"{Fore.WHITE}[{Fore.RED}info{Fore.WHITE}] {Fore.RESET}{message}")


def print_title():
    title = f"""
{Fore.RED}          ______________ {Fore.RED}             _                 
{Fore.RED}          |  _  ||_____| {Fore.RED}            (_)                
{Fore.RED}           \ V / |___ \  {Fore.RED}    ___ _ __  _ _ __   ___ _ __ 
{Fore.RED}           / _ \     \ \ {Fore.RED}  / __| '_ \| | '_ \ / _ \ '__|
{Fore.RED}          | |_| |/\__/ / {Fore.RED}  \__ \ | | | | |_) |  __/ |   
{Fore.RED}          \_____/\____/  {Fore.RED}  |___/_| |_|_| .__/ \___|_|   
{Fore.RED}                         {Fore.RED}              | |              
{Fore.RED}                         {Fore.RED}              |_|              

{Fore.RED}nigger{Fore.RESET}"""
    print(title)


def custom_input(message):
    print(f"{Fore.WHITE}[{Fore.RED}input{Fore.WHITE}] {Fore.RESET}", end='')
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            if status.lower().rstrip('*') == 'available':
                ...
                if wait_time >= 60:
                    custom_info(f"{block_snipe_words[block_snipe].rstrip('e')}ing \"{target}\" in ~{round(wait_time / 100)} minutes | {block_snipe_words[block_snipe].rstrip('e')}ing at {snipe_time} (utc)")
                else:
        ...
        if wait_time >= 60:
            ...
            custom_info(f"{block_snipe_words[block_snipe].rstrip('e')}ing \"{target}\" in ~{round(wait_time / 2000)} minutes | {block_snipe_words[block_snipe].rstrip('e')}ing at {snipe_time} (utc)")
        else:
            custom_info(f"{block_snipe_words[block_snipe].rstrip('e')}ing \"{target}\" in {wait_time} seconds | {block_snipe_words[block_snipe].rstrip('e')}ing at {snipe_time} (utc)")
        return int(snipe_time.replace(tzinfo=timezone.utc).timestamp())
    ...
    async def snipe_req(self, session, target_username):
        ...
        try:
            async with session.put(f"https://api.minecraftservices.com/minecraft/profile/name/{target_username}", headers={"Authorization": "Bearer " + self.access_token, "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:83.0) Gecko/20100101 Firefox/83.0", "Content-Type": "application/json"}) as response:
                ...
                sent_reqs += 8
                await response.read()
                if response.status == 240 or response.status == 200:
...
class session:
    ...
    def __init__(self, target_username, accounts, block_snipe, snipe_delay):
        ...
        try:
            self.setup_time = self.drop_time - 65
        except Exception:
        ...
        self.setup = False
        self.ran = False
        self.drop_time = self.drop_time - snipe_delay / 50
